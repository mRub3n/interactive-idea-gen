...
const Gallery = () => {
  ...
  return (
    <div className="min-h-screen pt-24 px-6 max-w-7xl mx-auto">
      <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="font-display text-4xl mb-8">
        Product Gallery
      </motion.h1>
      ...
      <div className="flex gap-2 mb-10 flex-wrap">
        {cats.map((c) => (
          <Button
            ...
            size="sm"
            className={`rounded-full ${filter === c.id ? "bg-accent text-accent-foreground hover:bg-accent/90" : ""}`}
      ...
      <motion.div layout className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
        {filtered.map((p) => (
          <motion.div key={p.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }}>
            <Link to={`/configure/${p.id}`} className="block group">
              <div className="glass rounded-3xl overflow-hidden hover:border-accent/30 transition-all duration-500">
                <div className="aspect-[4/3] bg-muted/20 flex items-center justify-center relative overflow-hidden">
                  <Box className="h-20 w-20 text-muted-foreground/15 group-hover:text-accent/25 transition-colors duration-500" />
